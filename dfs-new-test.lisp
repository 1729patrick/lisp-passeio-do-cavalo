...
;; TESTES COM NOVA ESTRUTURA
  (defun dfsearch-new (start-pos board &optional(max-depth 100))
	(dfs (list (list (make-node board start-pos 0 nil))
		  ) board max-depth)
  )
  
  (defun dfs-new (open graph max-depth)
  ;;debug print
		(terpri)
		(terpri)
		(print-list graph)
    ...
    (cond ((null open) nil)
        ...
        ;;((>(calculate-current-depth path) max-depth) (dfs (cdr open) graph max-depth))
              (t (dfs-new 
                  ...
                  (remove-simetric node (remove-node (node-value-taken-by-horse node) graph)) max-depth
                  ))
              ))
          )
          )
...
(defun successors-path (node)
  (let* (
         (line-column (node-coordinates node))
         )

    (cond ((or (null (first line-column)) (null (second line-column))) (list path))
          (t  (successors (first line-column) (second line-column) (parent-node node))
             ))
    )
  )
...
;; NIL
(defun successor-avaliable (line-index column-index current-node)
   (print current-node)
  (cond
   ((or (< line-index 0) (> line-index 9)) nil)
   ((or (< column-index 0) (> column-index 9)) nil)
  
    (t (make-node (node-state-board current-node) 
                    (list line-index column-index) 
                    (1+(depth-node current-node)) 
                    current-node)
      )
   )
  )
...
(defun successors (parent-node)
  (append
   (successor-avaliable (- (first (node-coordinates parent-node)) 2) (- (second (node-coordinates parent-node)) 1) parent-node)
   (successor-avaliable (- (first (node-coordinates parent-node)) 2) (+ (second (node-coordinates parent-node)) 1) parent-node)
   (successor-avaliable (+ (first (node-coordinates parent-node)) 2) (- (second (node-coordinates parent-node)) 1) parent-node)
   (successor-avaliable (+ (first (node-coordinates parent-node)) 2) (+ (second (node-coordinates parent-node)) 1) parent-node)
   (successor-avaliable (- (first (node-coordinates parent-node)) 1) (- (second (node-coordinates parent-node)) 2) parent-node)
   (successor-avaliable (- (first (node-coordinates parent-node)) 1) (+ (second (node-coordinates parent-node)) 2) parent-node)
   (successor-avaliable (+ (first (node-coordinates parent-node)) 1) (- (second (node-coordinates parent-node)) 2) parent-node)
   (successor-avaliable (+ (first (node-coordinates parent-node)) 1) (+ (second (node-coordinates parent-node)) 2) parent-node)
   )
